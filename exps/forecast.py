"""
Command-line entry point for the Stage 2 forecasting task.

The script loads a Stage 1 checkpoint into `M3RLStage2`, fine-tunes the
downstream model, and evaluates future metric prediction over the processed
metric/log/trace windows.
"""
...
from engines.forecast_engine import trainer, load_pretrained_weights
...
parser.add_argument("--task", type=str, default="forecast", help="Task name")
parser.add_argument(
...
def forecast(data_path, model_name, dataset_name, log, device, settings):
    ...
    for epoch in tqdm.tqdm(
        ...
        if epoch % valid_epoch_interval == 0:
            ...
            early_stopping(metrics_valid.mean_metrics["mse"], engine.model, save_path)
    ...
    log_string(
        log,
        "The Validation MSE of the best model is "
    ...
    for ix, data in tqdm.tqdm(
        ...
        metrics_test, samples, targets, sv = engine.test_model(
            x=x_test,
            y=y_test,
            x_mark_enc=None,
            x_mark_dec=None,
...
if __name__ == "__main__":
    ...
    start = time.time()
    forecast(data_path, settings.model, settings.dataset, log1, device1, settings)
    end = time.time()

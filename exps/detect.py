"""
Command-line entry point for the Stage 2 anomaly detection task.

The script evaluates system-level anomaly classification from the M3RL system
representation produced by the downstream model.
"""
...
from engines.detect_engine import trainer
...
def detect(data_path, model_name, dataset_name, log, device, settings):
    ...
    for epoch in tqdm.tqdm(
        ...
        for ix, data in tqdm.tqdm(
            ...
            if math.isnan(metrics_train.metrics["loss"]) or math.isnan(
                metrics_train.metrics["mse"]
            ):
                nan_count += 1
                logs = "nan occurs {:03d} times."
                log_string(log, logs.format(nan_count))
            elif (
                metrics_train.metrics["loss"] == float("inf")
                or metrics_train.metrics["mae"] == float("inf")
                or metrics_train.metrics["mse"] == float("inf")
            ):
                inf_count += 1
        ...
        if epoch % valid_epoch_interval == 0:
            ...
            his_loss.append(metrics_valid.mean_metrics["loss"])
            ...
            early_stopping(metrics_valid.mean_metrics["mae"], engine.model, save_path)
    ...
    log_string(
        log,
        "The Validation MAE of the best model is "
    ...
    for ix, data in tqdm.tqdm(
        ...
        metrics_test, samples, targets = engine.test_model(
            x=x_test,
            y=y_test,
            x_mark_enc=None,
            x_mark_dec=None,
    ...
    if not os.path.exists(save_path):
        os.makedirs(save_path)
    save_pkl_path = os.path.join(save_path, pkl_name)
    save_prediction_result(samples, targets, settings.predict_steps, save_pkl_path)
    ...
    if settings.if_wandb:
        ...
        wandb.finish()
    log_string(log, "Testing of {} is completed.".format(model_name))
...
if __name__ == "__main__":
    ...
    start = time.time()
    detect(data_path, settings.model, settings.dataset, log1, device1, settings)
    end = time.time()

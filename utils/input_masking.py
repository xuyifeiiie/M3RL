...
def mask_input(x, y, task_name, predict_steps, mix_masking_strategy):
    observed_mask = torch.ones_like(torch.cat([x, y], dim=1)).to(x.device)
    gt_mask = observed_mask.clone()
    gt_mask[:, -predict_steps:, :, :] = 0
    if task_name == "pretraining":
        if mix_masking_strategy == "equal_p":
            cond_mask = get_mask_equal_p_sample(observed_mask)
        elif mix_masking_strategy == "probabilistic_layering":
            cond_mask = get_mask_probabilistic_layering(observed_mask)
        else:
            print(
                "Please choose one of the following masking strategy in the config: equal_p, probabilistic_layering"
            )
    ...
    elif task_name == "Imputation with pattern":
        cond_mask = pattern_mask_batch(observed_mask)  # B T N F
    elif task_name.find("forecast") != -1:
        cond_mask = gt_mask  # B T N F  1 0
    else:
        print("Please choose the right masking to be applied during finetuning")
    cond_mask = cond_mask.to(x.device)
    target_mask = (observed_mask - cond_mask).to(x.device)
    return observed_mask, cond_mask, target_mask
...
def imputation_mask_batch(observed_mask):
    ...
    """
    rand_for_mask = (
        torch.rand_like(observed_mask) * observed_mask
    )  ### array like observed mask filled with random values
    rand_for_mask = rand_for_mask.reshape(len(rand_for_mask), -1)
    min_value, max_value = 0.1, 0.9
    for i in range(len(observed_mask)):
        sample_ratio = (
            min_value + (max_value - min_value) * np.random.rand()
        )  # missing ratio ## at random
...
def interpolation_mask_batch(observed_mask):
    ...
    for i in range(len(observed_mask)):
        mask_timestamp = np.random.choice(timestamps)
...
def forecasting_mask_batch(observed_mask):
    ...
    for i in range(len(observed_mask)):
        start_forecast_mask = np.random.choice(timestamps)
...
def forecasting_imputation_mask_batch(observed_mask):
    ...
    for i in range(len(observed_mask)):
        ...
        keep_dims_idx = random.sample(
            batch_indices, n_keep_dims
        )  # choose the dims to keep
        mask_dims_idx = [
            i for i in batch_indices if i not in keep_dims_idx
        ]  # choose the dims to mask
        start_forecast_mask = np.random.choice(timestamps)
